...
import type { SongKey, GraphNode, ListeningGraph, GraphFilter } from "./graph-types";
...
export function filterGraph(graph: ListeningGraph, filter: GraphFilter): ListeningGraph {
    ...
    for (const [key, node] of Object.entries(graph.nodes)) {
        ...
        if (clusterIds?.length && (node.clusterId === undefined || !clusterIds.includes(node.clusterId))) continue;
    ...
    for (const [key, node] of Object.entries(filteredNodes)) {
        ...
        prunedNodes[key] = { ...node, next: prunedNext as Record<SongKey, number>, previous: prunedPrevious as Record<SongKey, number> };
    }
 ...
 *
 * Nodes get attributes for rendering (size from PageRank, label, etc.).
 * Edges are directed with weight-based sizing.
 */
 ...
 */
export function toGraphology(listeningGraph: ListeningGraph): Graph<NodeAttributes, EdgeAttributes> {
