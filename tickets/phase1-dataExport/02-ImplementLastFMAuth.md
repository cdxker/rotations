...
- [ ] Config loading: read Last.fm API key and username from `.env`
- [ ] Validation: fail fast with a clear error if API key or username is missing
- [ ] Create a typed API client wrapper with the API key baked in (so fetchers don't need to manage auth)
- [ ] Test the auth by making a simple API call (e.g. `user.getInfo`)
